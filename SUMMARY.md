...
- **FSM Re-entrancy Fix**: The previous `checkAndSaveTokenThreshold()` method internally called `checkTokenThreshold()`, which only triggers from `IDLE` state. Since FSM was already in `CONFIRMED` after user reply, checkpoint saves were silently skipped — returning `{ triggered: false, saved: false }`. Fixed by using direct save path:
  ```typescript
  // BEFORE (broken):
