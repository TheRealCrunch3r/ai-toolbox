...
### `save_session_summary` 📝 💡 **v1.5.15+ Compressed Storage**
...
**Storage**: Payload is automatically compressed using `zlib.gzipSync(level: 9)` before storage (bypasses 10k SDK limit, ~30% size reduction). Legacy uncompressed summaries continue to work via backward-compatible fallback parser.
...
**Returns**: `{ success: true, data: { saved: boolean, summary_id: string, message: string, original_size_bytes: number, compressed_chars: number, preview: { task_description: string, timestamp: string } } }`

---

### `get_session_summary` 📖

Retrieve the most recent saved session summary for continuity across sessions. Automatically decompresses zlib-compressed payloads (v1.5.15+) with fallback to legacy uncompressed JSON format.
