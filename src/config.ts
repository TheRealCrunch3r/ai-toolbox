 ...
 * Returns the validated config if valid, throws on invalid input.
 */
...
/**
 * Checks if a specific tool category is enabled in the config.
 * @param config - The plugin configuration object
 * @param category - The tool category to check (must be a key of PluginConfig)
 * @returns true if the category is enabled, false otherwise
 */
export function isToolEnabled(config: PluginConfig, category: keyof PluginConfig): boolean {
