...
export const deTranslations: FullTranslationSet = {
  fileSystem: {
    ...
    tools: [
      ...
      { toolName: 'ripgrep', description: 'Rekursive Inhalts-Suche via worker-isoliertem ripgrep — der gesamte Scan läuft außerhalb des Host-Threads (3s-Watchdog beendet hängende Workers); case-insensitive standardmäßig (-i); ungültige Rust-Regexes werden automatisch als Literal wiederholt und über pattern_mode gemeldet; node_modules standardmäßig ausgeschlossen', parameters: ['pattern, path, mode, case_insensitive, include_glob, exclude_globs, max_depth'] },
  ...
  contextManagement: {
    ...
    tools: [
      ...
      { toolName: 'manage_projects', description: 'Cross-Projekt-Registry verwalten — Mutationen UND Lesezugriffe: registrieren / abmelden / aktualisieren / alle löschen + info / list / search (einziges Tool für ALLE Registry-Operationen; destruktive Aktionen erfordern confirm=true)', parameters: ['action (register|unregister|update|clear_all|info|list|search), project_name, working_dir_path (erforderlich für register/unregister/update/info), source_dirs (optional), query + max_results (für search), confirm (für destruktive Aktionen erforderlich)'] },
{ toolName: 'register_project', description: '[VERALTETES Alias von manage_projects(action="register")] Projekt im Cross-Projekt-Registry per Name + Working-Dir-Pfad registrieren oder aktualisieren (+ optionale Source-Dirs)', parameters: ['project_name, working_dir_path, source_dirs (optional)'] },
      { toolName: 'get_project_info', description: '[VERALTETES Alias von manage_projects(action="info")] Details eines registrierten Projekts per Working-Directory-Pfad abrufen', parameters: ['working_dir_path'] },
      { toolName: 'list_projects', description: '[VERALTETES Alias von manage_projects(action="list")] Alle registrierten Projekte mit Pfaden, letztem Zugriff und Session-Zählungen auflisten', parameters: [] },
      { toolName: 'search_projects', description: '[VERALTETES Alias von manage_projects(action="search")] Registrierte Projekte per Name- oder Pfad-Teilstring suchen; lazy Registry-Sync registriert automatisch in Session-Memory gefundene Projekte', parameters: ['query, max_results (optional)'] },
