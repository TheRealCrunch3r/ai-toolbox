...
export const enTranslations: FullTranslationSet = {
  fileSystem: {
    ...
    tools: [
      ...
      { toolName: 'ripgrep', description: 'Recursive content search via worker-isolated ripgrep — the whole scan runs off the host thread (3s watchdog terminates wedged workers); case-insensitive by default (-i); invalid Rust regexes auto-retry as literal, surfaced via pattern_mode; node_modules excluded by default', parameters: ['pattern, path, mode, case_insensitive, include_glob, exclude_globs, max_depth'] },
  ...
  contextManagement: {
    ...
    tools: [
      ...
      { toolName: 'manage_projects', description: 'Manage the cross-project registry — mutations AND reads: register / unregister / update / clear_all + info / list / search (single tool for ALL project-registry operations; destructive actions require confirm=true)', parameters: ['action (register|unregister|update|clear_all|info|list|search), project_name, working_dir_path (required for register/unregister/update/info), source_dirs (optional), query + max_results (for search), confirm (required for destructive actions)'] },
{ toolName: 'register_project', description: '[DEPRECATED alias of manage_projects(action="register")] Register or update a project in the cross-project registry by name + working-dir path (+ optional source dirs)', parameters: ['project_name, working_dir_path, source_dirs (optional)'] },
      { toolName: 'get_project_info', description: '[DEPRECATED alias of manage_projects(action="info")] Retrieve details of one registered project by its working directory path', parameters: ['working_dir_path'] },
      { toolName: 'list_projects', description: '[DEPRECATED alias of manage_projects(action="list")] List all registered projects with paths, last-accessed times and session counts', parameters: [] },
      { toolName: 'search_projects', description: '[DEPRECATED alias of manage_projects(action="search")] Search registered projects by name or path substring; lazy registry sync auto-registers projects found in session memory', parameters: ['query, max_results (optional)'] },
