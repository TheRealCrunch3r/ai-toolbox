...
export const esTranslations: FullTranslationSet = {
  fileSystem: {
    ...
    tools: [
      ...
      { toolName: 'ripgrep', description: 'Búsqueda de contenido recursiva con ripgrep en worker aislado — todo el escaneo se ejecuta fuera del hilo principal (watchdog de 3 s termina workers atascados); insensible a mayúsculas por defecto (-i); regex inválidos reintentan como literal y se reportan vía pattern_mode; node_modules excluido por defecto', parameters: ['pattern, path, mode, case_insensitive, include_glob, exclude_globs, max_depth'] },
  ...
  contextManagement: {
    ...
    tools: [
      ...
      { toolName: 'manage_projects', description: 'Gestiona el registro interproyectos — mutaciones Y lecturas: registrar / dar de baja / actualizar / borrar todos + info / list / search (herramienta única para TODAS las operaciones del registro; acciones destructivas requieren confirm=true)', parameters: ['action (register|unregister|update|clear_all|info|list|search), project_name, working_dir_path (requerido para register/unregister/update/info), source_dirs (opcional), query + max_results (para search), confirm (requerido para acciones destructivas)'] },
{ toolName: 'register_project', description: '[ALIAS OBSOLETO de manage_projects(action="register")] Registra o actualiza un proyecto en el registro interproyectos por nombre + ruta de directorio de trabajo (+ dirs fuente opcionales)', parameters: ['project_name, working_dir_path, source_dirs (opcional)'] },
      { toolName: 'get_project_info', description: '[ALIAS OBSOLETO de manage_projects(action="info")] Recupera los detalles de un proyecto registrado por su ruta de directorio de trabajo', parameters: ['working_dir_path'] },
      { toolName: 'list_projects', description: '[ALIAS OBSOLETO de manage_projects(action="list")] Lista todos los proyectos registrados con rutas, último acceso y recuentos de sesiones', parameters: [] },
      { toolName: 'search_projects', description: '[ALIAS OBSOLETO de manage_projects(action="search")] Busca proyectos registrados por nombre o subcadena de ruta; la sincronización perezosa del registro auto-registra proyectos encontrados en memoria de sesión', parameters: ['query, max_results (opcional)'] },
