...
export const zhCNTranslations: FullTranslationSet = {
  fileSystem: {
    ...
    tools: [
      ...
      { toolName: 'ripgrep', description: '通过 worker 隔离的 ripgrep 进行递归内容搜索——整个扫描在主线程之外执行（3 秒看门狗终止卡死的 worker）；默认不区分大小写（-i）；无效的 Rust 正则自动按字面量重试，并通过 pattern_mode 上报；默认排除 node_modules', parameters: ['pattern, path, mode, case_insensitive, include_glob, exclude_globs, max_depth'] },
  ...
  contextManagement: {
    ...
    tools: [
      ...
      { toolName: 'manage_projects', description: '管理跨项目注册表 — 变更与读取：注册 / 注销 / 更新 / 清空所有项目 + info / list / search（唯一工具，覆盖全部注册表操作；破坏性操作必需 confirm=true）', parameters: ['action (register|unregister|update|clear_all|info|list|search), project_name, working_dir_path（register/unregister/update/info 必需）, source_dirs（可选）, query + max_results（用于 search）, confirm（破坏性操作必需）'] },
{ toolName: 'register_project', description: '[已弃用的 manage_projects(action="register") 别名] 按名称 + 工作目录路径在跨项目注册表中注册或更新项目（+ 可选源码目录）', parameters: ['project_name, working_dir_path, source_dirs（可选）'] },
      { toolName: 'get_project_info', description: '[已弃用的 manage_projects(action="info") 别名] 按工作目录路径检索某个已注册项目的详情', parameters: ['working_dir_path'] },
      { toolName: 'list_projects', description: '[已弃用的 manage_projects(action="list") 别名] 列出所有已注册项目，含路径、最后访问时间与会话计数', parameters: [] },
      { toolName: 'search_projects', description: '[已弃用的 manage_projects(action="search") 别名] 按名称或路径子串搜索已注册项目；惰性注册表同步会自动注册在会话记忆中发现的项目', parameters: ['query, max_results（可选）'] },
