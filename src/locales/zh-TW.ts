...
export const zhTWTranslations: FullTranslationSet = {
  fileSystem: {
    ...
    tools: [
      ...
      { toolName: 'ripgrep', description: '透過 worker 隔離的 ripgrep 進行遞迴內容搜尋——整個掃描在主執行緒之外運行（3 秒看門狗終止卡死的 worker）；預設不區分大小寫（-i）；無效的 Rust 正規表示式自動以字面值重試，並透過 pattern_mode 回報；預設排除 node_modules', parameters: ['pattern, path, mode, case_insensitive, include_glob, exclude_globs, max_depth'] },
  ...
  contextManagement: {
    ...
    tools: [
      ...
      { toolName: 'manage_projects', description: '管理跨專案登錄表 — 變更與讀取：註冊 / 註銷 / 更新 / 清空所有專案 + info / list / search（唯一工具，涵蓋全部登錄表操作；破壞性操作必需 confirm=true）', parameters: ['action (register|unregister|update|clear_all|info|list|search), project_name, working_dir_path（register/unregister/update/info 必需）, source_dirs（選填）, query + max_results（用於 search）, confirm（破壞性操作必需）'] },
{ toolName: 'register_project', description: '[已棄用的 manage_projects(action="register") 別名] 依名稱 + 工作目錄路徑在跨專案登錄表中註冊或更新專案（+ 選填來源目錄）', parameters: ['project_name, working_dir_path, source_dirs（選填）'] },
      { toolName: 'get_project_info', description: '[已棄用的 manage_projects(action="info") 別名] 依工作目錄路徑擷取某個已註冊專案的詳細資訊', parameters: ['working_dir_path'] },
      { toolName: 'list_projects', description: '[已棄用的 manage_projects(action="list") 別名] 列出所有已註冊專案，含路徑、最後存取時間與工作階段計數', parameters: [] },
      { toolName: 'search_projects', description: '[已棄用的 manage_projects(action="search") 別名] 依名稱或路徑子字串搜尋已註冊專案；延遲登錄表同步會自動註冊在工作階段記憶中發現的專案', parameters: ['query, max_results（選填）'] },
