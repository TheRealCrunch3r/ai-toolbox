  ...
  if (metrics.includes('disk')) {
    try {
      ...
      if (platform === 'win32') {
        ...
        try {
          diskInfo = JSON.parse(psResult.stdout);
        } catch {
    ...
    implementation: async () => {
      try {
        ...
        for (const key of keys) {
          const value = await stateManager.get<string>(key);
          if (value !== undefined) {
            memories.push({
              id: key,
              fact: value,
      ...
      try {
        ...
        const idsWithTimestamps = await Promise.all(
          uniqueIds.map(async id => ({
            id,
            timestamp: await stateManager.get<number>(`session_summary_${id}_timestamp`) || 0,
          }))
        );
