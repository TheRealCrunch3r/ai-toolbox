...
describe('autoDetectAndRegister', () => {
  ...
  test('registers valid project with preferred name override', () => {
  ...
  test('uses detected name when preferred name is not provided', () => {
    const consoleLogSpy = jest.spyOn(console, 'log').mockImplementation();
...
describe('searchWithAutoRegister', () => {
  ...
  test('auto-registers cwd when registry is empty', async () => {
    const consoleLogSpy = jest.spyOn(console, 'log').mockImplementation();
    ...
    // Should have logged auto-detection message
    expect(consoleLogSpy).toHaveBeenCalledWith(
      expect.stringContaining('[ProjectAutoDetect] No projects found in registry')
    );

    consoleLogSpy.mockRestore();
  });

  test('logs warning when cwd is not a valid project', async () => {
    const consoleLogSpy = jest.spyOn(console, 'log').mockImplementation();

    fs.mkdirSync(testDir, { recursive: true });

    const results = await searchWithAutoRegister('test-project', testDir);
...
describe('initializeProjectDetection', () => {
  ...
  test('logs detected project info for valid projects', () => {
    const consoleLogSpy = jest.spyOn(console, 'log').mockImplementation();
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(
      '[ProjectAutoDetect] Detected project at startup:'
    );
    expect(consoleLogSpy).toHaveBeenCalledWith(
  ...
  test('logs non-project message for invalid directories', () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(
      '[ProjectAutoDetect] Current directory does not appear to be a project.'
    );
